...
from ANN.extra_sections.fourth_extra_section.utils.utils_extra_v4 import L_layer_model
...
with open(f"{get_path_of_documents()}/extra sections/fourth/report.txt", "w") as f:
    sum_of_trains = 0
    sum_of_tests = 0
    for i in range(5):
    ...
    f.write(f"\n\n----average cost in train data is {sum_of_trains / 50}-----")
    f.write(f"\n\n----average accuracy in test data is {sum_of_tests / (662 * 5)}-----")
